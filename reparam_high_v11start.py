...
# ctag = '{}-{}-{}'.format('vf5', 'a0', 65)
# coeff_file = cart_base + 'newpar_allatm/coeffs_finale_{}.p'.format(ctag)
# interp_coeffs = npl.precalc_interp_old(coeff_file = coeff_file, n_top = 65)
#
# hr_start_all = dict()
# for atm in allatms:
#     for cco2 in npl.allco2:
...
#
#         hr_start_all[(atm, cco2)] = npl.new_param_full_allgrids(temp, temp[0], pres, co2vmr, ovmr, o2vmr, n2vmr, interp_coeffs = interp_coeffs, old_param = True)[alt2-1]
#
# pickle.dump(hr_start_all, open(cart_out_rep + 'hr_start_all.p', 'wb'))
hr_start_all = pickle.load(open(cart_out_rep + 'hr_start_all.p', 'rb'))
...
for n_top in [57, 60, 63, 65, 67, 70, 75]:
    ...
    for afit, atmw in zip(['a{}s'.format(i) for i in range(5)], [np.ones(6), [0.3, 0.1, 0.1, 0.4, 0.05, 0.05], np.array([0.3, 0.1, 0.1, 0.4, 0.05, 0.05])**2, [0., 0.1, 0.1, 0., 1, 1], [1, 0., 0., 1, 0., 0.]]):
        atmweights = atmw
        ...
        for cco2 in range(1, npl.n_co2prof+1):
            hr_start = [hr_start_all[(atm, cco2)] for atm in allatms]
            result = least_squares(npl.delta_alpha_rec2_recf, start, args=(cco2, cose_upper_atm, alt2, n_top, atmweights, all_coeffs_nlte, atm_pt, name_escape_fun, hr_start, ), verbose=1, method = 'trf', bounds = bounds, max_nfev = 20000, ftol = 1.e-10, gtol = 1.e-10, xtol = 1.e-10)
    ...
    pickle.dump(alpha_dic, open(cart_out_rep + 'alpha_unif_allwstart_ntops.p', 'wb'))
